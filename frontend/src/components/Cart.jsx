    ...
    const fetchImagesAndUpdateCart = async () => {
      ...
      try {
        ...
        const updatedCartItems = cart
          ...
          .map((item) => {
            const backendProduct = backendProducts.find((p) => p.id === item.id);
            return {
              ...item,
              stockQuantity: backendProduct?.quantity ?? item.stockQuantity,
            };
          });
  ...
  const handleCheckout = async () => {
    try {
      for (const item of cartItems) {
        const updatedQuantity = item.stockQuantity - item.quantity;

        await axios.put(`http://localhost:8080/api/product/${item.id}`, {
          name: item.name,
          desc: item.desc,
          brand: item.brand,
          price: item.price,
